(function ($) {
  ...
  // Go to "Seus Pedidos" step if "Modify Order" is clicked
  ...
  var $menu = $("nav#menu").mmenu(
    {
      ...
      navbars: [
        {
          position: "bottom",
          content: ['<a href="#">© 2021 PAGAMENTO</a>'],
        },
      ],
    },
    {
  ...
  function resetModalOptions() {
    $(':radio[value="Médio: 32cm"]').prop("checked", true);
  ...
  // Function to show a popup essage that item is Adicionado no carrinho
  function showItemAddedMessage() {
    // Only show this message when there is no popup opened
    if (!$.magnificPopup.instance.isOpen) {
      // Show Adicionado no carrinho message
  ...
  function showItemAlreadyInCartMessage() {
    // Only show this message when there is no popup opened
    if (!$.magnificPopup.instance.isOpen) {
      // Show Já está  no carrinho message
  ...
  function setEmptyCart() {
    ...
    $("#emptyCart").html(
      '<div class="order-list-img"><img src="../img/bg/empty-cart-small.jpg" alt="Seu carrinho está vazio"/></div><div class="order-list-details"> <h4>Seu carrinho está vazio</a><br/><small>Comece adicionando intens</small></h4> <div class="order-list-price format-price">0.00</div></div>'
    );
    formatPrice();
  }
  ...
  function insertItemIntoCartRow(
    ...
    $("#cartItem" + id + rowId).html(
      '<div class="order-list-img"><img src="' +
        thumbnailPath +
  ...
  function addOptionsItemToCart(id) {
    ...
    itemPrice = $('input[name="size-options-item-' + id + '"]:checked')
      .nextAll(".option-price")
      .text();
  ...
  // Item having options is Adicionado no carrinho
  ...
  // Pure item without options is Adicionado no carrinho
